...
class LabelingWidget(LabelDialog):
    ...
    def __init__(  # noqa: C901
        ...
        mask_over_boxes = action(
            self.tr("Mask over Boxes"),
            self.mask_over_boxes,
            icon = "brain", #"brain"
            tip = self.tr("Generate instance masks from existing bounding boxes using SAM"),
        )
        mask_over_boxes_all_images = action(
            self.tr("Mask over Boxes (All)"),
            self.mask_over_boxes_all_images,
            None,
            "auto-run",
            self.tr("Run Mask over Boxes on all images"),
        )
        open_chatbot = action(
            self.tr("ChatBot"),
            self.open_chatbot,
    ...
    def mask_over_boxes(self):
        """
        Iterates over all rectangle shapes, uses them as prompts for the loaded SAM model,
        and generates new polygon masks with the same labels.
        """
        if not self.auto_labeling_widget.model_manager or not self.auto_labeling_widget.model_manager.loaded_model_config:
            self.error_message(
                self.tr("No Model Loaded"),
                self.tr("Please load a Segment Anything (SAM) model first.")
            )
            return
        ...
        if not rect_shapes:
            self.status(self.tr("No rectangle annotations found to process."))
            return

        if not self.auto_labeling_widget.model_manager.has_image_embedding():
            self.auto_labeling_widget.model_manager.set_image(self.image)

        new_shapes = []
        self.canvas.set_loading(True, self.tr("Processing boxes..."))

        try:
            for rect in rect_shapes:
                x_min = min(p.x() for p in rect.points)
                y_min = min(p.y() for p in rect.points)
                x_max = max(p.x() for p in rect.points)
                y_max = max(p.y() for p in rect.points)

                input_box = [x_min, y_min, x_max, y_max]

                result = self.auto_labeling_widget.model_manager.predict(
                    points=[],
                    point_labels=[],
                    rect=input_box
                )

                if result and result.shapes:
                    generated_shape = result.shapes[0]

                    new_shape = Shape(label=rect.label)
                    new_shape.points = generated_shape.points
                    new_shape.shape_type = 'polygon'
                    new_shape.flags = rect.flags
                    new_shape.group_id = rect.group_id

                    new_shapes.append(new_shape)
        except Exception as e:
            logger.error(f"Error in mask_over_boxes: {e}")
        finally:
        ...
        if new_shapes:
            ...
            self.set_dirty()
            self.status(self.tr(f"Generated {len(new_shapes)} masks from boxes."))

    def mask_over_boxes_all_images(self):
        """
        Run mask_over_boxes on all images in the file list.
        """
        if not self.auto_labeling_widget.model_manager or not self.auto_labeling_widget.model_manager.loaded_model_config:
            self.error_message(
                self.tr("No Model Loaded"),
        ...
        reply = QMessageBox.question(
            self,
            self.tr("Process All Images"),
            self.tr("Are you sure you want to run 'Mask over Boxes' on all images?"),
        ...
        progress = QProgressDialog(
            self.tr("Processing images..."), self.tr("Abort"), 0, len(self.image_list), self
        )
        ...
        for i, filename in enumerate(self.image_list):
            ...
            progress.setValue(i)
            progress.setLabelText(self.tr(f"Processing {i+1}/{len(self.image_list)}: {osp.basename(filename)}"))
    ...
    def mask_over_boxes(self):
        ...
        try:
            for rect in rect_shapes:
                ...
                result = self.auto_labeling_widget.model_manager.predict_shapes(
                    self.image,
                    self.filename,
                    batch=True
                )
